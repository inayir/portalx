...
@$username=$_GET['u']; //new \MongoDB\BSON\ObjectId($_GET['id']); 
if($username==''){ $filter=['$match'=>[ 'username'=>[ '$ne'=>null ] ] ];
}else{ $filter=['$match'=>[ 'username'=>$username ]]; }
...
if($cursor){  
	...
	foreach ($cursor as $formsatir) {  
		...
		$satir['changedata']=$cd;
		$satir['actdate']=$formsatir->actdate->toDateTime()->format($dtformat); 
		$fsatir[]=$satir;
		$fsay++;
	}
}
...
<head>
    ...
    <title><?php echo $gtext['s_personel'].":".$gtext['actions']; ?></title>
            ...
            <div id="content">
				...
				<!-- Page Heading -->
					<div class="align-middle justify-content-between mb-2 pt-1">
                        <h1 class="h4 text-left"><?php echo $gtext['s_personel'].": "; //
					  ...
					  <table class="table table-striped" id="fixtlist" width="100%" cellspacing="0">
						<thead>
						  <tr>
							...
							<TH class="word-wrap"><?php echo $gtext['name'];/*Hareket*/?></TH>
						...
						for($i=0; $i<$fsay; $i++){ 
							echo "<TR>\n";
							echo "   <TD>".$fsatir[$i]['action']."</TD>\n";
							echo "   <TD>".$fsatir[$i]['displayname']."</TD>\n";
