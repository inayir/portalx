            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
						...
						<a id="eklebtn" href="/AD/User_add.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" target="_blank"><i class="fas fa-user fa-sm text-white-50"></i> <?php echo $gtext['user']." ".$gtext['insert'];/*Kullanıcı Ekle*/?></a>
                    </div>
			...
			<div class="modal fade" id="upwModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="upwModalLabel"
				...
				<div class="modal-dialog modal-dialog modal-lg centered">
					<div class="modal-content">
						...
						<input class="form-control" type="hidden" name="usernamen" id="usernamen" value=""/>
						...
						<div class="modal-footer">
							<button class="btn btn-primary" id="userpbtn" disabled type="submit"><?php echo $gtext['save']; ?></button>
...
$(document).ready(function(){
	...
	//data:{ username: $('#usernamen').val(), pass: $('#ppass').val(), repass: $('#prepass').val(), rechpass: $('#rechpass').prop('checked') },
	var optfp={
		type	: 'POST',
		url 	: './set_pss.php',
		...
		beforesubmit: function(){ alert('test');
			if($('#ppass').val()!=$('#prepass').val()){ 
				return confirm('<?php echo $gtext['u_passnotsame'];?>'); 
			}
			return confirm($('#usernamen').val()+' şifresi değiştirilecek?');
		},
	...
	$("#userybtn").on("click", function(){ 
		var opt={
			...
			success: function(data){ 
				if(data=='login'){ alert('Please Login!'); location.assign('../login.php');}
				if(data.indexOf('!')>-1){ alert(data); location.reload(); }
				else { alert('!!!<?php echo $gtext['u_error'];?>'); }
			}
		}
